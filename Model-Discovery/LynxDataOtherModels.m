...
%A=[x1s x2s];
A=[x1s x2s x1s.^2 x1s.*x2s x2s.^2 x1s.^3 (x1s.^2).*x2s (x2s.^2).*x1s x2s.^3 x1s.^4 x2s.^4 x1s.^5 x2s.^5 one];
%A=[x1s x2s zilch zilch zilch zilch zilch zilch zilch zilch zilch zilch zilch one];

%A1=[x1s x2s zilch zilch zilch zilch zilch zilch zilch zilch zilch zilch zilch one];
%A2=[x1s x2s zilch zilch zilch zilch zilch zilch zilch zilch zilch zilch zilch one];
...
% xi1sparse=zeros(13,1);
% xi2sparse=zeros(13,1);
% xi1sparse(1)=xi1(1);
% %xi1sparse(2)=xi1(2);
% xi1sparse(4)=xi1(4);
% %xi1sparse(10)=xi1(10);
...
% xi2sparse(13)=xi2(13);
% 
% x1dotpredict = A1*xi1sparse;
% x2dotpredict = A2*xi2sparse;
...
BICHare=log(n)*parameters+n*log(2*pi)+n*log(varianceHare)+n
 ...
 ode1 = xi1(1)*y(1) + xi1(2)*y(2) + xi1(3)*y(1).^2 + xi1(4)*y(1)*y(2) + xi1(5)*y(2).^2 + xi1(6)*y(1).^3 + xi1(7)*(y(1).^2)*y(2) + xi1(8)*y(1)*(y(2).^2) + xi1(9)*y(2).^3 + xi1(10)*y(1).^4 + xi1(11)*y(2).^4 + xi1(12)*y(1).^5 + xi1(13)*y(2).^5 + xi1(14);
 ode2 = xi2(1)*y(1) + xi2(2)*y(2) + xi2(3)*y(1).^2 + xi2(4)*y(1)*y(2) + xi2(5)*y(2).^2 + xi2(6)*y(1).^3 + xi2(7)*(y(1).^2)*y(2) + xi2(8)*y(1)*(y(2).^2) + xi2(9)*y(2).^3 + xi2(10)*y(1).^4 + xi2(11)*y(2).^4 + xi2(12)*y(1).^5 + xi2(13)*y(2).^5 + xi2(14);
 odes = [ode1; ode2];
 end